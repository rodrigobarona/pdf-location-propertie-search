...
const MapWidget = ({ locationResult }: MapWidgetProps) => {
  ...
  const loadedPagesRef = useRef<Set<number>>(new Set([1])); // Track pages we've already requested
  ...
  useEffect(() => {
    ...
    // Reset loaded pages when location changes
        ...
        if (currentSearchId === searchId) {
          if (data.properties && Array.isArray(data.properties)) {
            ...
            if (hasMore && isAutoLoading) {
              // Use a slightly longer delay to avoid overwhelming the server
              setTimeout(() => {
  ...
  // Function to load more properties (manual trigger)
